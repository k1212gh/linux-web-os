...
/* ─── Context Menu ─── */
function ContextMenu({ x, y, onClose, apps }) {
  const { open, windows } = useWindowStore()
  const has = (id) => apps.some(a => a.id === id)
  const quickLinks = [
    has('terminal')    && { label: '⌨ 터미널 열기',     action: () => open('terminal') },
    has('claude-code') && { label: '🤖 Claude Code',      action: () => open('claude-code') },
    has('filemanager') && { label: '📂 파일 관리자',      action: () => open('filemanager') },
    has('monitor')     && { label: '📊 시스템 모니터',   action: () => open('monitor') },
    has('settings')    && { label: '⚙ 설정',             action: () => open('settings') },
  ].filter(Boolean)
  const items = [
    ...quickLinks,
    quickLinks.length > 0 && { type: 'divider' },
...
/* ─── Main Desktop ───
 * Props:
 *   apps: Array<{ id, title, icon, label, gradient, component }>
 *     — consumer builds the registry, so public builds omit private apps at
 *       compile time (security boundary enforced by import graph).
 */
export default function Desktop({ apps }) {
  ...
  useEffect(() => { APPS.forEach(app => registerApp(app)) }, [APPS])
    ...
    <>
      ...
      {booted && (
        <div style={{ position: 'relative', width: '100vw', height: '100vh', overflow: 'hidden', background: 'var(--bg-desktop)' }} onContextMenu={onContextMenu}>
          ...
          {/* Menus */}
          {ctxMenu && <ContextMenu x={ctxMenu.x} y={ctxMenu.y} onClose={() => setCtxMenu(null)} apps={APPS} />}
